...
ggplot(data = cor_matrix_melted, aes(x = Var1, y = Var2, fill = value)) +
  ...
  coord_fixed()
```
After reviewing the correlation matrix and calculating VIF values for each predictor, we observe that the 'displacement' variable has the highest VIF value (21.407632). A high VIF value indicates a strong correlation with other predictor variables, suggesting multicollinearity, which can inflate the variance of coefficient estimates and make the model unstable.
...
We observe that 'weight' variable has the highest VIF value this time (8.4456). We try removing 'weight' variable and see if that helps.
...
After removing 'weight' variable, the adjusted R-squared decreases significantly from 0.8079 to 0.762. Thus, we try removing predictor having the second highest VIF value (8.181615) which is 'horsepower' variable.
...
The result of removing 'horsepower' variable is actually way better than the one after removing 'weight' variable through the decrease of residual standard error by 0.002 and the climb of adjusted R-squared from 0.8079 to 0.8082. 
...
anova(model2, model3)
```
We use partial fisher test to compare model2 and model3. As p-value = 0.06318 > $\alpha$ = 0.05, we reject the reduced model, which is model2. The better model would be the full model, which is model3.
