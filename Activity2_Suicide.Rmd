...
```{r}
# Process 'year' column
data_clean$year <- data_clean$year - 1985
```
...
```{r}
model = lm(suicides_no ~ ., data = data_train)
summary(model)$r.squared
vif(model)
```

```{r}
model = lm(suicides_no ~ . - `countryUnited States` , data = data_train)
summary(model)$r.squared
vif(model)
```
```{r}
model = lm(suicides_no ~ . - `countryUnited States` - gdp_per_capita , data = data_train)
summary(model)$r.squared
vif(model)
```
