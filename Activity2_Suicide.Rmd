...
```
The missing of HDI.for.year account for about 70% of the dataset. Therefore, we decide to remove this variable. Moreover, country.year is combination of country and year; and suicides.100k.pop is suicides / (population / 100k) so we also decide to remove those variables
...
hist(log_suicides_no, breaks = 50)
boxplot(log_suicides_no)
```

```{r}
boxplot(log(population))
hist(gdp_per_capita, breaks = 20)
```
