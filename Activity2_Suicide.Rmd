...
```{r}
hist(log(data$suicides.100k.pop + 1)^2, breaks = 50)
boxplot(log(data$suicides.100k.pop +1))
```
...
best_lambda = 1.5
model_cox = lm((((data_train$log_suicides_no^best_lambda) - 1)/best_lambda) ~ . -transform_gdp_for_year -regionEurope -year, data = data_train)
summary(model_cox)
```
...
```{r}
# Use the Anderson-Darling test for normality
ad_test <- ad.test(residuals(model_cox))
print(ad_test)
```
...
formula <- as.formula(paste0("((log_suicides_no^", best_lambda, " - 1)/", best_lambda, ") ~ . -transform_gdp_for_year -regionEurope -year"))
...
predict_values <- exp((ifelse(predictions_log < 0, 0, predictions_log)  * best_lambda + 1)^(1/best_lambda) -1) -1
results <- data.frame(
