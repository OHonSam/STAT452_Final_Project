...
vif(model)
```
As it reduces a large amount of R-squared from ... to ... so we decide to keep regionEurope.
...
best_lambda = 1.25
model_cox = lm((((data_train$log_suicides_no^best_lambda) - 1)/best_lambda) ~ ., data = data_train)
summary(model_cox)
```
